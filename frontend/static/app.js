...
function productTemplate(product) {
  return `<article class="product"><h3>${product.name}</h3><p>${product.description}</p><p><strong>$${Number(product.price).toFixed(2)}</strong></p><p>❤️ ${product.likes} · 💾 ${product.saved}</p><div class="actions"><button data-action="like" data-id="${product.id}">Like</button><button data-action="save" data-id="${product.id}">Save</button><button data-action="cart" data-id="${product.id}">Add to Cart</button></div></article>`;
}

function cartTemplate(cart) {
  if (!cart.items.length) return "<p>Cart is empty.</p>";
  const lines = cart.items.map((item) => `<li>${item.name} × ${item.quantity} = $${Number(item.line_total).toFixed(2)}</li>`).join("");
